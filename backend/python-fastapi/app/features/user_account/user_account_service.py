...
class UserAccountService:
    ...
    async def check_privilege_by_username(
        self, user_account: UserAccountResponse, username: str
    ) -> bool:
        if user_account.username == username:
            return True
        if await self.user_account_repository.is_user_account_private(
            username=username
        ):
            if not await self.user_account_repository.is_follower(
    ...
    async def get_by_username(self, username: str) -> UserAccountResponse:
        user_account = await self.user_account_repository.get_by_username(
            username=username
        )
        user_account_response = UserAccountResponse.from_orm(user_account)
        return user_account_response
    ...
    async def get_list_of_followers_by_username(
        ...
        user_accounts = (
            await self.user_account_repository.get_list_of_followers_by_username(
                username=username
            )
        )
    ...
    async def get_list_of_following_by_username(
        ...
        user_accounts = (
            await self.user_account_repository.get_list_of_following_by_username(
                username=username
            )
        )
        return [UserAccountResponse.from_orm(x) for x in user_accounts]

    async def is_private(self, username: str) -> bool:
        return await self.user_account_repository.is_user_account_private(
            username=username
        )

    async def is_follower(self, requester_username: str, target_username: str) -> bool:
        requester_user_account = await self.get_by_username(username=requester_username)
        target_user_account = await self.get_by_username(username=target_username)
